...
class Sample:
    ...
    """
    def __init__(self, state, action, reward, next_state, is_terminal): ## return copied
        self.state = state
        self.action = action
        self.reward = reward
...
class Preprocessor:
    ...
    def process_batch(self, samples):
        ...
        raise NotImplementedError('This method should be overridden in Atari')
...
class ReplayMemory:
    ...
    def __init__(self, config):
        ...
        self.states = np.zeros((self.memory_size, config.screen_height, config.screen_width), dtype = np.uint8)
    ...
    def append(self, state, action, reward, isterminal):  ## copied
        index = self.count % self.memory_size
        statecp = np.copy(state)
        self.states[index] = np.array(statecp, dtype=np.uint8)
        self.actions[index] = np.array(np.uint8(action))
        self.rewards[index] = np.float32(reward)
        self.terminals[index] = np.bool(isterminal)
        self.count += 1

    def sample(self, batch_size): ## retrun non-copied
        poolsize = min(self.count, self.memory_size)
        indexes = []
        while len(indexes) < batch_size:
            index = np.random.randint(0, poolsize)
            indexes.append(index)
        samples = []
        for index in indexes:
            if index < self.history_length - 1:
                resi_index = self.memory_size - (self.history_length - 1 - index)
                resi = np.copy(self.states[resi_index:])
                resi_terminal = self.terminals[resi_index:]
                state = np.zeros((self.history_length, self.dims[0], self.dims[1]), dtype = np.uint8)
                state[0:(self.history_length - 1 - index)] = resi
                state[(self.history_length - 1 -index):] = self.states[:index+1]
                # state = np.copy(np.append(resi, self.states[:index+1]))
                tmpterminal = np.append(resi_terminal, self.terminals[:index+1])
            else:
                state = np.copy(self.states[index-3:index+1])
                tmpterminal = self.terminals[index-3:index+1]
            for t_index in range(self.history_length-1): ## clean up for up to first 3 states
                if tmpterminal[t_index]:
                    state[0: t_index+1] = np.zeros((t_index+1, self.dims[0], self.dims[1]), dtype = np.uint8)
            isterminal = self.terminals[index]
            action = self.actions[index]
            reward = self.rewards[index]
            nexstate = np.append(state[1:], [self.states[(index+1) % poolsize]], axis=0)

            assert len(tmpterminal) == self.history_length
            assert len(state) == self.history_length
            assert len(nexstate) == self.history_length
            sample = Sample(state, action, reward, nexstate, isterminal)
    ...
    def clear(self):
        ...
        self.states = np.empty((self.memory_size, self.dims[0], self.dims[1]), dtype = np.uint8)
