...
class DoubleDQNAgent:
    ...
    def select_action(self, state, **kwargs):
        ...
        q_values_target = self.q_target.predict(state_pre, batch_size=1)[0]
        for _index in range(len(q_values)):
            q_values[_index] = (q_values[_index] + q_values_target[_index])/2
    ...
    def fit(self, env, num_iterations, max_episode_length=None):
        ...
        while cnt < num_iterations:
            ...
            action = self.select_action(current_state)  # Get action
            ...
            if cnt >= self.num_burn_in and cnt % self.train_freq == 0:          # update
                coinflip = np.random.randint(0, 2)   # 0:q 1:q_target
                if coinflip:
                    Q0 = self.q_target
                    Q1 = self.q
                else:
                ...
                y = np.zeros((self.batch_size, action_size), dtype=np.float32)
                for _index in range(len(samples)):
                    sample = samples[_index]
                    x[_index] = np.copy(sample.state)
                    if sample.is_terminal:
                        y[_index] = self.calc_q_values(sample.state, Q0)
                        y[_index][sample.action] = sample.reward
                    else:
                        y[_index] = self.calc_q_values(sample.state, Q0)
                        _action = np.argmax(self.calc_q_values(sample.next_state, Q0))  # Argmax Q0
                        q_next = self.calc_q_values(sample.next_state, Q1)[_action]   # Use Q1[argmaxQ0] to update
                        y[_index][sample.action] = sample.reward + self.gamma*q_next
                if coinflip:
                    mse_loss, mae_metric = self.q_target.train_on_batch(x, y)
                else:
            ...
            if cnt % (num_iterations/3) == 0:  # Save model
                ...
                self.q.save_weights(str(self.config.modelname) + '_model_1_' + TimeStamp+'_weights.h5')
                self.q_target.save_weights(str(self.config.modelname) + '_model_2_' + TimeStamp+'_weights.h5')
